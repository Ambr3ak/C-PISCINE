...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/07/22 20:43:20 by abourdar          #+#    #+#             */
/*   Updated: 2020/07/23 08:48:50 by abourdar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (*str)
	{
		if (!ft_strchr(charset, *str))
		{
			...
			while (str[size] && !ft_strchr(charset, str[size]))
				size++;
		tab[index] = (char *)malloc((size + 2) * sizeof(char));
...
char **ft_split(const char *str, const char *charset)
{
	...
	if (!(tab = (char **)malloc((size + 2) * sizeof(char *))))
		return (NULL);
